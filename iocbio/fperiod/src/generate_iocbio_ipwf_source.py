...
class Generator:
    def __init__(self, pwf, extension='cutoff', form=''):
        """
        Parameters
        ----------
    ...
    def integrate(self, integrand='f(x)*f(x+y)', extension='cutoff'):
        ...
        if extension=='periodic':
            ...
            if self.form=='normalized_derivative':
                # normalized_integral = (N-y)*diff(integral,y) - integral
                integral1 = integral1.variable_diff(r)*(eval('R.Number(N-j)-r', self.namespace)) - integral1
                integral2 = integral2.variable_diff(r)*(eval('R.Number(N-j)-r', self.namespace)) - integral2
                integral3 = integral3.variable_diff(r)*(eval('R.Number(N-j)-r', self.namespace)) - integral3
            elif self.form=='':
                pass
            else:
  ...
  return -1.0;
    ...
    def generate_approximation_source(self):
        ...
        for poly_order in range (8):
            ...
            cf_proto = 'void %(cfunc_prefix)squadratic_approximation_3_%(poly_order)s(%(decl_coeffs1)s, %(decl_coeffs2)s, %(decl_coeffs3)s, double* p0, double* p1, double* p2)' % (locals ())
...
%(cf_proto)s
{
  ...
  *p2 = %(p2)s;
            ...
            cf_proto = 'void %(cfunc_prefix)squadratic_approximation_1_%(poly_order)s(%(decl_coeffs1)s, double* p0, double* p1, double* p2)' % (locals ())
...
%(cf_proto)s
{
  ...
  *p2 = %(p2)s;
            ...
            cf_proto = 'void %(cfunc_prefix)slinear_approximation_3_%(poly_order)s(%(decl_coeffs1)s, %(decl_coeffs2)s, %(decl_coeffs3)s, double* p0, double* p1)' % (locals ())
...
%(cf_proto)s
{
  ...
  *p1 = %(p1)s;
            ...
            cf_proto = 'void %(cfunc_prefix)slinear_approximation_1_%(poly_order)s(%(decl_coeffs1)s, double* p0, double* p1)' % (locals ())
...
#define F(I) ((I)<0?(f[(I)+n]):((I)>=n?f[(I)-n]:f[(I)]))
  ...
  %(set_coeffs)s
        ...
        for order in range(poly_order+1):
            ...
            order_cases.append('case %(order)s:  %(cfunc_prefix)s%(name)s_compute_coeffs_diff%(order)s(j, fm, n, m, %(coeffs)s); break;' % (locals()))
            ...
            order_cases_extreme.append('case %(order)s: return %(cfunc_prefix)s%(name)s_find_extreme_diff%(order)s(j0, j1, fm, n, m, result);' % (locals()))
...
%(cf_proto2)s
{
  ...
  for (j=start_j; j<end_j; ++j)
  {
    %(set_coeffs_ref21)s
    %(set_coeffs_ref32)s
    %(cfunc_prefix)s%(name)s_compute_coeffs_diff%(order)s(j, fm, n, m, %(refcoeffs3)s);
    count ++;
    if (count<3)
      continue;
    %(cfunc_prefix)squadratic_approximation_3_%(poly_order2)s(%(coeffs1_2)s, %(coeffs2_2)s, %(coeffs3_2)s, &p0, &p1, &p2);
    if (p2!=0.0)
    {
       s = -0.5*p1/p2;
       if (s>=0.0 && s<=1.0)
...
%(cf_proto2)s
{
  ...
  for (j=start_j; j<end_j; ++j)
  {
    ...
    %(cfunc_prefix)squadratic_approximation_1_%(poly_order2)s(%(coeffs1_2)s, &p0, &p1, &p2);
    if (p2!=0.0)
    {
       s = -0.5*p1/p2;
       if (s>=0.0 && s<=1.0)
  ...
  return status;
            ...
            coeffs2_2 = ','.join(coeffs2.split(',')[:poly_order2+1])
            coeffs3_2 = ','.join(coeffs3.split(',')[:poly_order2+1])
            order_cases_zero.append('case %(order)s: return %(cfunc_prefix)s%(name)s_find_zero_diff%(order)s(j0, j1, fm, n, m, result, slope);' % (locals()))
            cf_source_template2_3 = '''
%(cf_proto2)s
{
  int count = 0;
  ...
  for (j=start_j; j<end_j; ++j)
  {
    ...
    if (count<3)
      continue;
    %(cfunc_prefix)slinear_approximation_3_%(poly_order2)s(%(coeffs1_2)s, %(coeffs2_2)s, %(coeffs3_2)s, &p0, &p1);
    if (p1!=0.0)
    {
       s = -p0/p1;
...
%(cf_proto2)s
{
  ...
  {
    %(cfunc_prefix)s%(name)s_compute_coeffs_diff%(order)s(j, fm, n, m, %(refcoeffs1)s);
    %(cfunc_prefix)slinear_approximation_1_%(poly_order2)s(%(coeffs1_2)s, &p0, &p1);
...
%(cf_proto)s
{
  ...
  return -2;
    ...
    function %(name)s_find_extreme(j0, j1, f, n, m, order, res) result (status)
       ...
       fortranname %(cfunc_prefix)s%(name)s_find_extreme
...
%(cf_proto)s
{
  ...
  return -2;
        ...
        '''
        pyf_template = '''
    function %(name)s_find_zero(j0, j1, f, n, m, order, res, slope) result (status)
       intent(c) %(name)s_find_zero
       fortranname %(cfunc_prefix)s%(name)s_find_zero
       integer intent(in, c) :: j0, j1
       double precision dimension (m, n), intent(in,c):: f
       integer, depend(f), intent(c,hide) :: n = (shape(f,1)==1?shape (f,0):shape(f,1))
       integer, depend(f), intent(c,hide) :: m = (shape(f,1)==1?1:shape(f,0))
  ...
  */
  ...
  switch (order)
  {
    ...
    function %(name)s_evaluate(y, f, n, m, order) result (value)
       ...
       fortranname %(cfunc_prefix)s%(name)s_evaluate
...
#define F(I) ((I)<0?((1-(I))*f[0]+(I)*f[1]):((I)>=n?(((I)-n+2)*f[n-1]-((I)-n+1)*f[n-2]):f[(I)]))
  ...
  return 0.0;
    ...
    function %(name)s_f%(k)s_evaluate (x, f, n, order) result (value)
      ...
      fortranname %(cfunc_prefix)s%(name)s_f%(k)s_evaluate
