...
double fperiod_compute_period(double* f, int n, int m, double structure_size)
{
  ...
  period = fperiod_find_acf_maximum(r, n, m, lbound, ubound);
#else
  k = 1;
  for(j=0; j<m; ++j)
    {
...
#define ISLTONE(r) ISNONNEG(1-(r))
  ...
  for (j=lbound+1;j<ubound;++j)
    {
      c = c1;
      compute_coeffs_1(j+1, f, n, m, &c1);
      if (!(ISNONNEG(c) && ISNONPOS(c1)))
	continue;
      ...
      if (ISNONNEG(r) && ISLTONE(r))
	{
	  ...
	  y = (double)j + r;
	  //eps = 0;
	  //t = 0.2*a*eps*eps*eps*eps/(eps*eps*(a*r+b)+b*r*r+2*c*r+d);
	  //printf("r=%f, t=%f\n", r, t);
	  //y = y + t;
	}
    }
