...
sl = mpl_lines.Line2D.filled_markers * 1000
fl = mpl_lines.Line2D.fillStyles * 1000
cl = ['Indigo', 'CornflowerBlue', 'Gold', 'DarkGreen', 'LimeGreen', 
      'HotPink', 'OrangeRed', 'Crimson', 'Black'] * 800
...
def make_plot_list_old(args, x_list, y_list, species_list):
...
def make_plot_list(args, x_list, y_list, species_list):
    ...
    sol_dic = {}
    for met_name, series, nlabels, colour, symbol, fill, xl, solution in plot_list:
        if met_name == 'CP':
            sol_dic[(met_name, nlabels)] = solution

    pts = [0.02, 0.13, 0.26]
    vls = [99, 99, 99]
    for index, cp1 in enumerate(sol_dic[('CP',1)]):
        cp2 = sol_dic[('CP',2)][index]
        cpT = (cp1 + 2*cp2) / (3*0.3)
        sol_dic[('CP',0)][index] = cpT
        if cpT > pts[0] and vls[0] == 99:
            vls[0] = cpT
        if cpT > pts[1] and vls[1] == 99:
            vls[1] = cpT
        if cpT > pts[2] and vls[2] == 99:
            vls[2] = cpT
            
    sol_dic[('CP',3)] = copy.deepcopy(sol_dic[('CP', 0)])
    sol_dic[('CP',5)] = copy.deepcopy(sol_dic[('CP', 0)])

    for index, cp in enumerate(sol_dic[('CP', 3)]):
        sol_dic[('CP', 3)][index] = sol_dic[('CP', 0)][-1]

    xvls = [99, 99, 99]
    for index, cp in enumerate(sol_dic[('CP', 0)]):
        tol = 1e-8
        if abs(cp - vls[0]) < tol:
            xvls[0] = x_list[0][index]
        if abs(cp - vls[1]) < tol:
            xvls[1] = x_list[0][index]
        if abs(cp - vls[2]) < tol:
            xvls[2] = x_list[0][index]

    #vls.append(0)
    #xvls.append(0)
            
    A = numpy.vstack([vls, numpy.ones(len(vls))]).T
    B = numpy.array(xvls)
    B = B[:, None]
    
    m, c = numpy.linalg.lstsq(A, B)[0]
    
    sol_dic[('CP',4)] = vls

    print m, c

    for index, cp in enumerate(sol_dic[('CP', 5)]):
        v = (x_list[0][index]-c) / m 
        if v < sol_dic[('CP',0)][-1]:
            sol_dic[('CP',5)][index] = v
        else:
            sol_dic[('CP', 5)][index] = sol_dic[('CP', 0)][-1]

    
    for index, (met_name, series, nlabels, colour, symbol, fill, xl, solution) in enumerate(plot_list):
        if met_name == 'CP' and nlabels == 0:
            sol = sol_dic[('CP', 0)]
            hsol = sol_dic[('CP', 3)]
            fit = sol_dic[('CP', 5)]
            psol = sol_dic[('CP', 4)]
            plot_list[index] = (met_name, series, nlabels, colour, symbol, fill, xl, sol)
            #plot_list.append((met_name, series, 3, colour, '', fill, xl, hsol))
            plot_list.append((met_name, series, 5, colour, '', fill, xl, fit))
            plot_list.append((met_name, series, 4, colour, 'X', fill, xvls, psol))
...
def slice_soln(args, series, nlabels, x_values, solution):
    ...
    if args.plot_type in ['tr', 'lm', 'fp', 'ftp']:
        tol = 1e-5
...
def plot_output2(y_list=None, x_list=None, species_list=None,
                plot_name='default',
                args=None,
                legend_columns=1,
                title=None,
    ...
    for index, data in enumerate(sorted(plot_list)):
        ...
        if args.plot_type == 'tr':
            ...
            pyplot.xlim(xmin, xmax)
        if symbol == 'X':
            ax.plot(x_values, solution, 'o', color='red')
        else:
            ax.plot(x_values, solution, color=colour, linestyle='solid', linewidth=1)
...
def add_model_parameter_arguments(p, string_arg='S', float_arg='F',
    ...
    p.add_argument("-H", "--frac-W",
                   dest="frac_W", default=0.3, metavar=FL, type=float,
                   help="Value of frac_W, the fraction of labeled water for the simulation. {0}".format(d))
...
def make_solution_name(args, include_list=None):
    if include_list is None:
        include_list = range(13)

    test_arguments(args)
    s = 'X'
    nd = dict(A=s, B=s, C=s, D=s, E=s, F=s, G=s, H=s, J=s, K=s, L=s, M=s, N=s)
    for index in include_list:
        if index == 0: nd['A'] = round_float(args.AS_flux * 10000)
        if index == 1: nd['B'] = round_float(args.frac_W * 10000)
        if index == 2: nd['C'] = round_float(args.frac_ATP * 10000)
        if index == 3: nd['D'] = round_float(args.frac_AK * 10000)
        if index == 4: nd['E'] = round_float(args.frac_CK * 10000)
        if index == 5: nd['F'] = round_float(args.ef_AKi * 10000)
        if index == 6: nd['G'] = round_float(args.ef_AKo * 10000)
        if index == 7: nd['H'] = round_float(args.ef_CKi * 10000)
        if index == 8: nd['J'] = round_float(args.ef_CKo * 10000)
        if index == 9: nd['K'] = round_float(args.ef_ASs * 10000)
        if index == 10: nd['L'] = round_float(args.ef_ASe * 10000)
        if index == 11: nd['M'] = args.bidirectional_transport_case
        if index == 12: nd['N'] = args.pool_case

    ns = Namespace()
    ns.__dict__ = nd
    
    return '{0.A}_{0.B}_{0.C}_{0.D}_{0.E}_{0.F}_{0.G}_{0.H}_{0.J}_{0.K}_{0.L}_{0.M}_{0.N}'.format(ns)
...
def test_arguments(args):
    ...
    elif args.frac_W < 0 or args.frac_W > 1:
        msg = "The fraction of labeled water must be between 0 and 1."
        raise UserWarning(msg, (args.frac_W))
...
def decode_solution_name(args, solution_name):
    ...
    if len(e) != 13:
        msg = "A solution_name was passed that did not have thirteen data elements separated by '_' characters."
        raise UserWarning(msg, (solution_name, e))
    
    args.AS_flux = float(e[0]) / 10000.0
    args.frac_W = float(e[1]) / 10000.0
    args.frac_ATP = float(e[2]) / 10000.0
    args.frac_AK = float(e[3]) / 10000.0
    args.frac_CK = float(e[4]) / 10000.0
    args.ef_AKi = float(e[5]) / 10000.0
    args.ef_AKo = float(e[6]) / 10000.0
    args.ef_CKi = float(e[7]) / 10000.0
    args.ef_CKo = float(e[8]) / 10000.0
    args.ef_ASs = float(e[9]) / 10000.0
    args.ef_ASe = float(e[10]) / 10000.0
    args.bidirectional_transport_case = e[11]
    args.pool_case = e[12]

    args.model_name = 'model_{0}_mass'.format(e[1])
